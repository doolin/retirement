...
module Retirement
  # Computes Required Minimum Distributions using the
  # IRS Uniform Lifetime Table (SECURE 2.0, age 73+).
  module Rmd
    RMD_START_AGE = 73
    ...
    def rmd_factor(age)
      return 0.0 if age < RMD_START_AGE

      clamped = [age, UNIFORM_TABLE.keys.max].min
      UNIFORM_TABLE.fetch(clamped, 2.0)
    end

    def compute_rmd(pretax_balance, age)
      factor = rmd_factor(age)
      return 0.0 if factor <= 0

      pretax_balance / factor
    end
  end
end
